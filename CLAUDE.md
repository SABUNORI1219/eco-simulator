...
**「cons≈gen（Trio A確定コストとエメラルド生産量が近い）領地が多かったことが中央値張り付きの原因」という仮説は、実データで否定された（2026-08調査）。** `scratchpad/phase5d`配下の保存済みスナップショット7件（highf 1件・midf 6件、いずれも取得直後10分以内の領地・Trio A確定不能な領地を除外）を対象に、判定可能な延べ2,833領地について`|cons_em − gen_em| / max(cons_em, gen_em, 1)`（相対差）を集計した。**相対差20%未満の領地はわずか7件（0.2%）、10%未満は0件だった。** 残り95.5%（2,707件）は相対差80%以上であり、`cons_em`と`gen_em`が近い領地はごく少数の例外に過ぎず、多数派ではない。20%未満の7件（各1件ずつ、重複領地名なし）を具体的に見ると、最小の相対差はToxic Drip（17.50%、`Efficient Resources Lv.0 × Resource Rate Lv.0`＝コンボ強化なし、`Larger Resource Storage Lv.5`のみでcons=16000）で、他の6件（Crater Descent/Disturbed Crypt/Fort Tericen/Little Wood/Raiders' Stronghold/Corkus City/Cinfras Outskirts/Maro Peaks/Durum Isles Barn、うち相対差最小順に7件を採用）も相対差33〜53%の範囲にとどまり、20%未満はToxic Drip1件のみだった。したがって、実データでは「典型的な構成ならcons≈genにはならない」という直感のほうが支持され、**中央値方式が破綻した原因はcons≈gen領地の多発ではなく、別の要因（`stored_em=0`の領地が単独最大クラスタとして約半数を占めたこと自体、またはXP Seeking未計上）と考えるべきである。** この結果は「グリッド探索＋exactlyOne最大化方式を維持する」という上記の結論を変えるものではない。
